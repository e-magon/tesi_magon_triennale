...
% Per compilare il documento leggere le istruzioni nel file BUILDING.md
...
% Mostra i bounding box per visualizzare il layout
% \geometry{showframe}
...
\usepackage{pgfplotstable} % Per leggere dati da CSV
\usepackage{csvsimple} % Per creare tabelle da CSV
...
\unilogo{immagini/loghi/unimi_noalpha}
...
%   TITOLO TESI:
\title{Sensitive Data Detector Identificazione automatica di dati sensibili all'interno di log}
...
\printedtitle{Sensitive Data Detector:\\
Identificazione automatica di\\
dati sensibili all'interno di log}
...
% Numera i listing sequenzialmente (1, 2, 3...) come figure e tabelle,
% evitando conflitti con le sezioni dell'appendice (A.1 vs Sorgente A.1)
\makeatletter
\AtBeginDocument{%
...
Ringrazio il Professore Marco Anisetti e il Dottore Antongiacomo Polimeno per la disponibilità e il supporto offerti durante la ricerca, lo sviluppo e la stesura di questa tesi. \\
Ringrazio inoltre la mia famiglia, i miei amici e soprattutto Nicol per il sostegno e la pazienza dimostrati durante tutto il mio percorso universitario.
...
I log possono assumere forme eterogenee, spaziando da semplici righe di testo non strutturato a rappresentazioni complesse basate su formati standardizzati. Dal punto di vista del contenuto, le registrazioni possono limitarsi a tracciare in modo sintetico la sequenza degli eventi nel sistema oppure possono includere descrizioni dettagliate degli stati interni, dei dati in ingresso e in uscita, delle informazioni immesse dagli utenti durante l'interazione con il sistema e dei parametri di configurazione.
...
Contenendo informazioni dettagliate sul funzionamento interno dei sistemi, i log rappresentano un obiettivo privilegiato per gli utenti malevoli. Ciò si manifesta su due fronti distinti: da un lato, pratiche di logging poco attente espongono frequentemente informazioni personali identificabili (\textit{PII}) come indirizzi email, credenziali e identificatori di dispositivo. Studi su applicazioni Android hanno rilevato che circa il 6\% delle app analizzate presenta fughe di dati personali nei log. Ciò rende i log un bersaglio appetibile per attacchi informatici finalizzati al furto di informazioni~\cite{aghili2024protecting_privacy_logs, hou2025catamaran}.

Dall'altro lato, la manomissione dei log rappresenta una pratica diffusa: indagini nel campo della risposta agli incidenti riportano che il 72\% degli specialisti ha riscontrato log alterati o cancellati da parte degli attaccanti~\cite{paccagnella2020custos}. Tali attività di \textit{anti-forensics} mirano a ostacolare le indagini eliminando le tracce delle intrusioni~\cite{jena2012audit_log_hash}, mentre in ambienti \textit{IoT} la compromissione dei dispositivi consente agli avversari di modificare, eliminare o iniettare eventi nei log per eludere l'attribuzione delle responsabilità~\cite{koisser2023accountability_iot_logging}.
...
\textbf{Microsoft Presidio}\footnote{Microsoft Presidio \url{https://microsoft.github.io/presidio}} è un \textit{SDK} per la protezione e la de-identificazione di dati personali che offre moduli separati per l'analisi del testo, l'anonimizzazione, il trattamento di immagini e i dati strutturati. Il sistema combina tecniche di \textit{Named Entity Recognition}\footnote{Named Entity Recognition (NER) è una tecnica di elaborazione del linguaggio naturale che identifica e classifica automaticamente entità nominate all'interno di un testo, come nomi di persone, organizzazioni, luoghi, date ed espressioni temporali.}, espressioni regolari, regole basate su contesto e checksum per individuare entità \textit{PII} (ad esempio numeri di carte di credito, indirizzi, codici identificativi) e applicare trasformazioni quali mascheramento, sostituzione o cifratura. La piattaforma è estendibile e integra diversi motori \textit{NLP}\footnote{NLP (Natural Language Processing) è l'elaborazione del linguaggio naturale, un campo dell'intelligenza artificiale che si occupa dell'interazione tra computer e linguaggio umano, permettendo ai sistemi di comprendere, interpretare e generare testo in modo automatico.} e servizi cloud, ma è pensata principalmente come componente generico di data protection e non come soluzione specializzata per flussi continui di log.

\textbf{anonympy}\footnote{anonympy \url{https://pypi.org/project/anonympy}} è una libreria Python orientata all'anonimizzazione di dati tabellari, immagini e PDF. Supporta diverse tecniche per dati di tipologie differenti, come dati numerici e temporali, e include funzionalità per oscurare automaticamente informazioni sensibili presenti in documenti PDF o immagini. La natura di libreria Python la rende facilmente integrabile in software personalizzati, tuttavia il focus primario resta l'elaborazione di dataset strutturati e documenti: per applicarla ai log di testo libero sarebbe necessario introdurre una fase di trasformazione preliminare.

\textbf{Data Protection Framework}\footnote{Data Protection Framework \url{https://github.com/thoughtworks-datakind/anonymizer}} è una libreria e applicazione a riga di comando sviluppata da ThoughtWorks per l'identificazione, l'anonimizzazione e la de-anonimizzazione di dati \textit{PII} in file CSV. Il \textit{framework} adotta un approccio ibrido simile a Presidio, combinando pattern basati su espressioni regolari con riconoscimento di entità tramite NLP. Il software è progettato per essere eseguito in pipeline di elaborazione dati, ma risulta ancora incompleto e fortemente orientato a dati tabellari.

\textbf{NgAnonymize}\footnote{NgAnonymize \url{https://github.com/sonbachmi/NgAnonymize}} è una libreria per applicazioni Angular 2+ che offre funzioni di anonimizzazione direttamente a livello di interfaccia utente, tramite una \textit{pipe} che può \textit{randomizzare}, mescolare o mascherare porzioni di stringhe. Lo strumento è utile per nascondere dati personali in viste \textit{front-end} (ad esempio nomi, indirizzi o numeri di telefono), ma non include alcun meccanismo di rilevamento automatico: è lo sviluppatore a dover identificare e marcare esplicitamente i campi da anonimizzare. Di conseguenza, NgAnonymize non è adatto all'analisi automatica di grandi volumi di log generati da sistemi eterogenei.
...
Prima di elaborare un input testuale un LLM deve convertere il testo ricevuto in \textbf{token}, che rappresentano le unità base di elaborazione. Un token può corrispondere a una parola intera, una porzione di parola (come una sillaba), o anche singoli caratteri, a seconda del metodo di \textit{tokenizzazione} adottato. Il \textbf{\textit{context window}} definisce il numero massimo di token che il modello può elaborare in una singola interazione: ad esempio, un \textit{context window} di 8.192 token consente al modello di analizzare circa 6.000-8.000 parole contemporaneamente, a seconda della lingua e della complessità del testo. All'interno del \textit{context window} rientra non solo l'input corrente, ma anche l'intera cronologia della conversazione in corso (comprensiva di tutti i messaggi precedenti e delle relative risposte del modello) e le eventuali istruzioni di sistema. Questo limite determina quindi quanta informazione complessiva il modello può considerare simultaneamente durante l'elaborazione di una risposta.

Il numero di \textbf{parametri} di un modello, espresso tipicamente in miliardi (\textit{B}), rappresenta invece la quantità di connessioni neurali che compongono il modello. Un modello con 7 miliardi di parametri (7B) ha memorizzato 7 miliardi di \textbf{\textit{pesi}} che determinano come elaborare e interpretare il testo. Modelli con più parametri sono generalmente più adatti a comprendere sfumature linguistiche complesse e a gestire compiti diversificati, ma richiedono anche maggiori risorse di memoria e potenza di calcolo per funzionare.

\subsection{Machine learning}
\label{subsec:machine_learning}

Il \textbf{machine learning} rappresenta una branca dell'intelligenza artificiale che consente ai sistemi informatici di apprendere e migliorare le proprie prestazioni attraverso l'esperienza, senza essere esplicitamente programmati per ogni specifica situazione~\cite{shinde2018review_ml_dl}. Le sue origini risalgono agli anni '50 del XX secolo, quando ricercatori come Alan~Turing e\\
Arthur~Samuel iniziarono a esplorare la possibilità di creare macchine capaci di apprendere autonomamente.
...
La ricerca accademica ha evidenziato che, sebbene i modelli proprietari tendano a mostrare performance superiori in \textit{benchmark} generali, modelli \textbf{\textit{open weight}} opportunamente ottimizzati possono raggiungere risultati comparabili in task specifici come la comprensione di testi~\cite{alassan2024comparison_opensource_proprietary}. La valutazione delle performance deve inoltre considerare non solo l'accuratezza, ma anche aspetti pratici come la riproducibilità dei risultati e la trasparenza dei processi di valutazione~\cite{white2024livebench}. In particolare, \textit{benchmark} come \textbf{LiveBench} sono stati sviluppati per fornire valutazioni più eque e resistenti alla contaminazione del test set, problematica che può favorire artificialmente modelli addestrati su dataset che includono i dati di test~\cite{white2024livebench}. Per un confronto comparativo dei modelli proprietari e \textit{open weight}, si rimanda alla Figura~\refwithpage{fig:livebench_comparison}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{immagini/white2024livebench-1.jpg}
    \caption{Confronto delle performance di modelli proprietari e \textit{open weight} su LiveBench~\cite{white2024livebench}}
    \label{fig:livebench_comparison}
\end{figure}

\clearpage
\section{Large Language Model nell'analisi dei log}
\label{sec:llm_analisi_log}
...
In questo contesto, i \textbf{Large Language Model} rappresentano un cambio di paradigma nell'approccio all'analisi dei log, introducendo capacità di comprensione contestuale e flessibilità semantica. Studi recenti mostrano che attraverso l'uso di LLM per il parsing dei log, con tecniche di \textit{few-shot tuning}\footnote{Per \textit{few-shot tuning} si intende un riaddestramento mirato del modello su un numero molto ridotto di esempi etichettati del compito specifico, tipicamente poche decine o centinaia di istanze.}, è possibile raggiungere una parsing accuracy media intorno al 96\% su 16 sistemi open-source eterogenei~\cite{ma2024llmparser}, mentre \textit{benchmark} dedicati all'analisi di log applicativi e di sicurezza evidenziano che modelli \textbf{Transformer} \textit{fine-tuned}\footnote{Un modello \textit{fine-tuned} è un modello che è stato riaddestrato in maniera mirata per un determinato scopo.} ottimizzati ulteriormente sul compito di analisi dei log possono superare lo stato dell'arte con un \textit{F1-Score} medio di 0{,}998, ovvero la media armonica\footnote{La media armonica di due valori $a$ e $b$ è definita come $\frac{2ab}{a+b}$. A differenza della media aritmetica, quella armonica penalizza maggiormente i valori bassi: se uno dei due termini è molto piccolo, la media armonica risulta significativamente inferiore. Ad esempio, un classificatore con \textit{precision} = 0{,}95 e \textit{recall} = 0{,}10 avrebbe una media aritmetica di 0{,}525, ma una media armonica (\textit{F1-Score}) di soli 0{,}18, evidenziando lo sbilanciamento delle prestazioni.} tra \textbf{precision} e \textbf{recall}\footnote{La \textit{precision} misura la proporzione di elementi previsti come positivi che sono effettivamente positivi (pochi falsi positivi), mentre la \textit{recall} misura la proporzione di elementi positivi correttamente individuati dal modello (pochi falsi negativi).}, metrica ampiamente utilizzata per valutare modelli di classificazione~\cite{karlsen2023benchmarking_llm_log}.

Nonostante queste notevoli prestazioni, l'applicazione efficace dei LLM ai log richiede spesso un adattamento specifico al dominio. I log di sistema utilizzano terminologia tecnica, abbreviazioni e pattern sintattici che differiscono significativamente dal linguaggio naturale su cui i modelli sono tipicamente addestrati, generando un evidente \textbf{\textit{domain gap}}. Per affrontare questo problema, alcuni lavori propongono strategie di \textbf{\textit{continual pre-training}} su conoscenza interpretabile del dominio: il modello \textbf{SuperLog}, ad esempio, viene adattato a partire da LLM \textit{open weight} ottenendo un miglioramento medio del 12{,}01\% in accuracy rispetto al secondo miglior modello su quattro task diversi di analisi dei log~\cite{ji2024adapting_llm_log}.
...
L'architettura proposta consiste in un sistema di analisi dei log, progettato per ricevere flussi di registrazioni, elaborarle e classificarle in base alla presenza o assenza di dati sensibili. Il sistema può operare sia in modalità \textit{streaming}, processando ciascuna riga di log non appena questa viene acquisita, sia in modalità \textit{batch}, analizzando più righe di log in una volta sola. In entrambi i casi, il sistema restituisce un indicatore dell'esito dell'analisi.
...
Il secondo livello sfrutta le capacità di comprensione contestuale dei \textit{Large Language Model} per analizzare dinamicamente il contenuto testuale delle righe di log. Questa componente è progettata per intercettare informazioni sensibili espresse in linguaggio naturale o che non seguono schemi sintattici predefinibili, compensando i limiti dell'analisi statica attraverso una valutazione semantica del contenuto.
...
L'approccio metodologico del progetto prevede l'utilizzo dei \textit{Large Language Model} sia come strumento di supporto durante lo sviluppo per la definizione e validazione delle espressioni regolari, sia come componente attiva integrata nel processo di analisi in tempo reale.
...
L'approccio proposto integra un sistema che combina l'uso di espressioni regolari con l'utilizzo di \textit{Large Language Model}, creando una soluzione ibrida per l'identificazione di dati sensibili in tempo reale.

Il primo livello ha un ruolo di \textbf{analisi sintattica}, cioè di identificazione rapida e deterministica di \textit{pattern} strutturati ben definiti mediante espressioni regolari, come spiegato nella sezione~\refwithpage{sec:approccio_identificazione}.
...
L'elaborazione inizia con l'\textbf{acquisizione}, durante la quale i log vengono ricevuti dal sistema centralizzato di gestione. Segue la \textbf{pre-elaborazione sintattica}, in cui un primo \textit{layer} applica espressioni regolari ottimizzate per identificare \textit{pattern} comuni e ad alta occorrenza; i log contenenti corrispondenze positive vengono immediatamente marcati come sensibili.

Successivamente avviene la fase di \textbf{instradamento}, dove i log vengono inoltrati al sistema di analisi semantica. Quest'ultimo procede con l'\textbf{analisi contestuale}, durante la quale il modello linguistico esamina ogni riga di log per identificare dati sensibili non rilevati dall'analisi sintattica iniziale, varianti e formattazioni non standard di dati già categorizzati, informazioni sensibili espresse in linguaggio naturale, e contesti in cui dati apparentemente innocui assumono natura sensibile.
...
\begin{table}[h!]
    ...
    \begin{tabular}{|p{0.11\textwidth}|p{0.25\textwidth}|p{0.27\textwidth}|p{0.25\textwidth}|}
        ...
        \hline
        Host+PID      & Stringa + `[PID]:' & Nome host e \textit{process ID} del processo che ha generato il log     & `sw3-devaccess|55dacabaa 607[561]:' \\
        \hline
        Body          & Oggetto JSON       & Contenuto strutturato con `\textit{level}' e `\textit{message}'         & `\{"level":"info",\newline"message":"..."\}' \\
        \hline
    \end{tabular}
...
Il livello di elaborazione costituisce il cuore del sistema ed è responsabile della \textbf{raccolta}, \textbf{gestione} e \textbf{analisi} dei log. Le tecnologie in questo \textit{layer} sono state selezionate per garantire \textbf{scalabilità}, \textbf{affidabilità} e \textbf{precisione} nell'identificazione dei dati sensibili.
...
La scelta di Graylog è stata motivata principalmente dal fatto che questa piattaforma viene già utilizzata nell'ambiente di produzione per il quale è stato sviluppato \textit{Sensitive Data Detector}, rendendo quindi l'integrazione di quest'ultimo una naturale estensione dell'infrastruttura esistente.

Oltre al vantaggio dell'integrazione con l'ambiente esistente, Graylog presenta diverse caratteristiche tecniche che lo rendono ideale per questo progetto. La piattaforma offre un sistema di \textbf{raccolta centralizzata} con supporto nativo per diversi protocolli di logging, tra cui GELF\footnote{GELF (Graylog Extended Log Format) è un formato strutturato progettato specificamente per il trasporto di log su reti TCP/UDP, che estende il formato Syslog standard con campi aggiuntivi e supporto nativo per metadati.} e Syslog\footnote{Syslog è un protocollo standard per il logging di sistema definito dall'RFC 3164 (\url{https://datatracker.ietf.org/doc/html/rfc3164}), ampiamente utilizzato per la trasmissione di messaggi di log su reti IP.}, permettendo l'integrazione con sistemi eterogenei. Un esempio di messaggio GELF è riportato nel sorgente~\refwithpage{lst:esempio_gelf}.

\lstinputlisting[
caption=Esempio di messaggio GELF,
label=lst:esempio_gelf,
literate={è}{{\`e}}1 {à}{{\`a}}1 {ò}{{\`o}}1 {ù}{{\`u}}1 {ì}{{\`i}}1 {µ}{{$\mu$}}1
]{"../example_logs/gelf.json"}

La capacità di \textbf{indicizzazione \textit{real-time}} consente di rendere i log immediatamente ricercabili dopo la ricezione, mentre il sistema di \textbf{\textit{pipeline} di \textit{processing}} fornisce regole configurabili per trasformare, filtrare e arricchire i log durante la fase di \textit{ingestion}.\\
Le \textbf{API REST complete} offrono interfacce programmatiche per l'automazione della configurazione e l'integrazione con sistemi esterni. Infine, la \textbf{scalabilità orizzontale} garantita dall'architettura distribuita supporta \textit{cluster} multi-nodo per gestire volumi elevati di log.
...
Ollama\footnote{Ollama \url{https://ollama.com}} è un software open-source progettato per semplificare l'esecuzione \textbf{locale} di Large Language Model.\\
...
\begin{table}[h!]
    ...
    \begin{tabular}{|p{0.15\textwidth}|p{0.18\textwidth}|p{0.52\textwidth}|}
        \hline
        \textbf{Nome} & \textbf{Parametri} & \textbf{Caratteristiche principali}                        \\ \hline
        ...
        Qwen 2.5      & 7B                 & Modello multilingue addestrato su dati di svariati tipi    \\ \hline
...
I modelli DeepSeek R1 includono capacità di \textbf{reasoning}\footnote{Il reasoning negli LLM si riferisce alla capacità di generare esplicitamente il processo di ragionamento step-by-step che porta alla risposta finale. Questi modelli utilizzano tag speciali (come \texttt{<think>...</think>}) per separare il processo di ragionamento interno dalla risposta definitiva, permettendo maggiore trasparenza nel processo decisionale.} avanzate che permettono di visualizzare il processo di "\textit{ragionamento}" interno del modello.

Ogni modello viene personalizzato attraverso file di configurazione \textit{Modelfile} che definiscono diversi aspetti fondamentali. Nello specifico, il \textit{Modelfile} definisce il \textbf{\textit{prompt} di sistema}, contenente istruzioni specifiche per l'identificazione di dati sensibili nei log, e altri \textbf{parametri ottimizzati}, che configurano la temperatura\footnote{La temperatura è un parametro che controlla la casualità delle risposte dell'LLM: valori bassi (0.0-0.3) producono risposte più deterministiche e conservative, mentre valori alti (0.7-1.0) aumentano la creatività ma riducono la coerenza.} e il \textit{context window}\footnote{Il limite massimo di \textit{token} elaborabili in una singola interazione; per una definizione approfondita si rimanda alla sezione~\refwithpage{sec:llm}.} per massimizzare la precisione. Gli \textbf{esempi di \textit{training}} guidano il modello nel formato di \textit{output} atteso, mentre le \textbf{definizioni precise} specificano in dettaglio cosa costituisce un dato sensibile e cosa escludere dall'analisi.
...
Questa componente del sistema è stata implementata in tre versioni evolutive che rappresentano l'evoluzione del progetto dalla dimostrazione di fattibilità all'integrazione enterprise.

La \textbf{prima versione} costituisce un \textit{proof of concept} per validare l'uso di LLM nell'identificazione di dati sensibili.
...
La \textbf{terza versione} implementa l'integrazione con Graylog, sia in modalità \textit{push} che \textit{pull}, per deployment in ambiente di produzione, permettendo quindi sia l'elaborazione \textit{stream} in real-time che quella \textit{batch} per l'analisi di log pre-esistenti.

Per una descrizione completa dell'architettura, delle funzionalità e dei risultati di ogni versione si rimanda al \textbf{Capitolo~\refwithpage{chap:sensitive_data_detector}}.

Il file \texttt{\textbf{graylog\_setup.py}} automatizza la configurazione iniziale di Graylog tramite le API REST per la creazione \textbf{programmatica} di input, stream, output e pipeline; adotta una configurazione \textbf{idempotente} che consente esecuzioni ripetute senza effetti collaterali. Include inoltre meccanismi di gestione degli errori con validazione delle configurazioni e, dove previsto, rollback automatico.
...
Il codice completo dello script è disponibile nel sorgente~\refwithpage{lst:code_graylog_script}.

\subsection{Docker}
\label{subsec:docker}

Docker\footnote{Docker \url{https://www.docker.com}} viene utilizzato per la \textit{containerizzazione} dell'ambiente Graylog, garantendo portabilità, isolamento e semplicità di deployment.

La configurazione \textbf{Docker Compose} definisce un ambiente completo composto da tre servizi principali: \textbf{MongoDB}\footnote{MongoDB \url{https://www.mongodb.com}}, che funge da \textit{database} per la memorizzazione dei metadati di configurazione di Graylog; \textbf{Graylog DataNode}, il nodo di indicizzazione responsabile dello \textit{storage} e della ricerca dei log; e \textbf{Graylog Enterprise}, il server principale che espone l'interfaccia web e le API REST.

Il codice completo della configurazione dei container è disponibile nel sorgente~\refwithpage{lst:code_container_config}.

L'ambiente Docker implementa diverse soluzioni per garantire sicurezza e affidabilità. Una \textbf{rete isolata}, realizzata tramite \textit{bridge network} dedicata, assicura la comunicazione sicura tra i container. I \textbf{volumi persistenti} garantiscono lo \textit{storage} permanente per i dati di MongoDB, gli indici di Graylog e le configurazioni. Il \textbf{\textit{port mapping}} consente l'esposizione selettiva delle sole porte necessarie per l'integrazione con sistemi esterni. Infine, gli \textbf{\textit{health checks}} permettono il monitoraggio automatico dello stato dei servizi, attivando le \textit{restart policies} configurate in caso di malfunzionamenti.
...
Il progetto \textit{Sensitive Data Detector} è il risultato dell'implementazione dell'architettura proposta ed è stato sviluppato attraverso un \textbf{processo iterativo} che ha portato alla realizzazione di tre versioni evolutive, ognuna con obiettivi specifici e caratteristiche tecniche distinte.

Questo approccio progressivo ha permesso di affrontare le sfide tecniche in modo sistematico, partendo dalla validazione del concetto di base fino all'implementazione di una soluzione pronta per l'utilizzo in \textbf{produzione}. Ogni versione introduce significativi miglioramenti in termini di scalabilità, accuratezza e facilità di integrazione in ambienti esistenti rispetto alla precedente.

\subsubsection{Versione 1: Proof of Concept}

La prima implementazione costituisce un \textit{proof of concept} sviluppato per dimostrare la fattibilità dell'utilizzo di Large Language Model nell'identificazione automatica di dati sensibili all'interno dei file di log.

Questa versione utilizza un'architettura \textbf{sincrona} con elaborazione \textbf{\textit{batch}}, interfacciandosi direttamente con l'API Ollama per analizzare file di log pre-esistenti. Il sistema impiega LLM base senza personalizzazione, guidati esclusivamente da un \textit{user-prompt} iniziale e implementa strategie di gestione manuale del \textit{context window} con ripristino periodico.
...
\subsubsection{Versione 2: Valutazione quantitativa}

La seconda versione rappresenta un'evoluzione significativa con focus sulla scalabilità e la valutazione \textbf{quantitativa} delle performance. Introduce l'elaborazione \textbf{riga per riga} dei log per ottimizzare l'uso della memoria e utilizza modelli LLM personalizzati creati tramite \textit{Modelfile} di Ollama, che incorporano prompt di sistema specifici per l'identificazione di dati sensibili.

Un aspetto fondamentale di questa versione è l'implementazione di un sistema di \textbf{\textit{ground truth}}\footnote{\textit{Ground truth} indica i dati di riferimento considerati corretti e utilizzati come standard per valutare la precisione di un sistema di classificazione. Nel contesto di questo progetto, consiste in etichette manuali che indicano se ogni riga di log contiene o meno dati sensibili.} per la valutazione automatica, che consente il calcolo di metriche di classificazione \textbf{binaria} (veri/falsi positivi/negativi) e delle metriche derivate accuratezza complessiva, \textit{precision}, \textit{specificity} e \textit{recall}, oltre all'analisi comparativa di diversi modelli LLM.

Ulteriori dettagli sono disponibili nella sezione~\refwithpage{sec:ver2}.

\subsubsection{Versione 3: Integrazione enterprise}

La terza versione costituisce la soluzione \textit{production-ready} del progetto, implementando un'architettura \textbf{asincrona} completa per l'integrazione con Graylog sia in modalità \textit{push} che \textit{pull}.\\
Utilizza la programmazione asincrona con \texttt{asyncio} per gestione concorrente delle connessioni, il protocollo GELF per comunicazione nativa con la piattaforma di gestione log, un server TCP non bloccante per la ricezione dei log tramite \textit{stream} in tempo reale (in modalità \textit{push}) e chiamate REST per l'acquisizione \textit{batch} incrementale dei log (in modalità \textit{pull}).

Il sistema implementa un \textbf{\textit{feedback loop}} bidirezionale che reinvia automaticamente i risultati delle analisi a Graylog, permettendo la visualizzazione unificata di log originali e notifiche di rilevamento di dati sensibili attraverso le sue dashboard specializzate.
...
La prima versione di \textit{Sensitive Data Detector} rappresenta un \textit{proof of concept} sviluppato per dimostrare la fattibilità dell'utilizzo di Large Language Model nell'identificazione automatica di dati sensibili all'interno dei file di log.\\
...
Il prompt iniziale è stato progettato per guidare il modello nell'identificazione precisa dei dati sensibili, fornendo \textbf{definizioni chiare} di cosa costituisce un dato sensibile, \textbf{esempi specifici} di dati da identificare come token, credenziali e informazioni personali, \textbf{esclusioni esplicite} per dati non sensibili (quali username, ID e timestamp), un \textbf{formato di output strutturato} in bullet point e un \textbf{esempio pratico} con input e output atteso.
...
Il prompt completo è disponibile nel sorgente~\refwithpage{lst:prompt_ver1}.
...
La distinzione dei parametri riflette le diverse capacità computazionali e di \textit{context window} dei modelli. I modelli più grandi con 70 miliardi di parametri possono processare blocchi più ampi di dati e mantenere conversazioni più lunghe prima del ripristino del contesto.

I valori relativi al numero di righe per messaggio e alla frequenza di ripristino della \textit{chat} sono stati determinati attraverso un processo di \textbf{calibrazione empirica}. Durante la fase di sperimentazione si è osservato che, impostando valori troppo elevati per questi parametri, i modelli LLM tendevano a ``dimenticare'' progressivamente il \textit{prompt} iniziale man mano che la conversazione si allungava. Questo fenomeno, noto come \textit{context degradation}, causava un deterioramento della qualità delle risposte: il modello iniziava a ignorare le istruzioni originali, producendo output non conformi al formato richiesto o identificando erroneamente dati non sensibili come sensibili. Il ripristino periodico della conversazione, reinviando il prompt iniziale, si è rivelato essenziale per mantenere l'aderenza alle specifiche durante elaborazioni prolungate.
...
\section{Versione 2: Valutazione quantitativa}
\label{sec:ver2}

La seconda versione di \textit{Sensitive Data Detector} rappresenta un'evoluzione significativa del \textit{proof of concept}, introducendo un sistema di valutazione automatica delle performance e l'utilizzo di modelli LLM personalizzati ottimizzati per il compito specifico.

Questa versione si concentra sull'elaborazione efficiente di file di log di dimensioni maggiori tramite streaming delle righe e implementa metriche di valutazione quantitative. Per ulteriori informazioni riguardanti la valutazione oggettiva, si rimanda al Capitolo~\refwithpage{chap:test}.

\subsubsection{Obiettivi e scopo}
\label{subsubsec:ver2_obiettivi}

L'obiettivo principale di questa seconda implementazione è duplice: da un lato migliorare la scalabilità del sistema per gestire volumi di log più elevati, dall'altro introdurre un \textit{framework} di valutazione automatica per misurare obiettivamente le performance dei modelli LLM.

La versione introduce diversi miglioramenti sostanziali: l'\textbf{elaborazione riga per riga} per ottimizzare l'uso della memoria e semplificare il processo di rilevazione della correttezza delle risposte, l'\textbf{utilizzo di modelli personalizzati} con prompt engineering avanzato, un \textbf{sistema di \textit{ground truth}} per la valutazione automatica e il \textbf{calcolo di metriche di classificazione binaria} quali veri/falsi positivi/negativi e metriche derivate accuratezza complessiva, \textit{precision}, \textit{specificity} e \textit{recall}.
...
Il sistema utilizza modelli LLM personalizzati creati tramite \textit{Modelfile} che incorporano prompt di sistema specifici e configurazioni ottimizzate per l'identificazione di dati sensibili. Per ulteriori informazioni riguardanti i \textit{Modelfile}, si rimanda alla sezione~\refwithpage{subsec:llm}.\\
L'implementazione completa è disponibile nel sorgente~\refwithpage{lst:code_ver2}.

Il flusso di elaborazione ottimizzato si articola nelle seguenti fasi:

\begin{enumerate}
    \item \textbf{Inizializzazione del dataset}: Il sistema carica file di log pre-annotati con etichette \textit{ground truth} (Y/N) per ogni riga
    ...
    \item \textbf{Preprocessamento}: Le etichette di \textit{ground truth} vengono estratte e conservate per la valutazione, mentre il contenuto della riga di log viene inviato al modello
    ...
    \item \textbf{Valutazione in tempo reale}: Ogni risposta viene immediatamente confrontata con il \textit{ground truth} e le metriche vengono aggiornate
...
A differenza della prima versione, dove il prompt iniziale veniva inviato ad ogni interazione, i modelli personalizzati incorporano le istruzioni direttamente nella loro configurazione tramite \textit{Modelfile}.

Questa strategia offre vantaggi significativi: garantisce \textbf{consistenza} mantenendo il prompt di sistema sempre presente nel contesto del modello ed eliminando la variabilità dovuta alla gestione manuale, migliora l'\textbf{efficienza} riducendo la lunghezza dei messaggi inviati e ottimizzando l'utilizzo del \textit{context window}, permette la \textbf{specializzazione} ottimizzando ogni modello con prompt specifici per le sue caratteristiche architetturali e supporta il \textbf{reasoning avanzato} per modelli con capacità di ragionamento che utilizzano tag speciali come \texttt{<think>}.

Il sistema implementa anche un meccanismo di post-processing per gestire le risposte dei modelli reasoning, rimuovendo automaticamente i tag di \textit{reasoning} che contengono il processo di ragionamento interno del modello, considerando solamente la risposta finale.
...
Tutti i modelli condividono una configurazione unificata ottimizzata per l'elaborazione sequenziale di singole righe di log, come mostrato nella tabella~\refwithpage{tab:modelli_llm_ver2}. La struttura dei Modelfile è disponibile nel sorgente~\refwithpage{lst:modelfile}.
...
A differenza della prima versione, tutti i modelli utilizzano una configurazione unificata con elaborazione di una riga per messaggio e ripristino della conversazione ogni 15 messaggi. Questa standardizzazione facilita il confronto diretto delle \textit{performance} tra modelli diversi e ottimizza l'utilizzo delle risorse computazionali.

La scelta di elaborare una singola riga alla volta presenta vantaggi metodologici significativi. Innanzitutto, questo approccio garantisce una \textbf{risposta univoca} da parte dell'LLM per ogni unità di log analizzata: il modello produce una classificazione binaria diretta (sensibile/non sensibile) senza ambiguità interpretative. Al contrario, inviando molteplici righe in un unico messaggio, sarebbe necessario implementare meccanismi di \textit{parsing} per associare correttamente ogni porzione della risposta alla riga di log corrispondente. Inoltre, l'elaborazione \textit{batch} di più righe simultaneamente incrementa il rischio di \textbf{risposte incomplete o errate}: l'LLM potrebbe omettere involontariamente alcune righe nella sua analisi, produrre output con formattazione inconsistente, o perdere il contesto necessario per una classificazione accurata. L'approccio unitario elimina queste problematiche, garantendo al contempo una valutazione immediata dell'accuratezza delle classificazioni prodotte dal modello.
...
\begin{itemize}
    \item \textbf{Metadati dell'esecuzione}: timestamp, modello utilizzato, file di log analizzato e numero totale di righe processate
    \item \textbf{Metriche di classificazione}: veri positivi, veri negativi, falsi positivi, falsi negativi
    \item \textbf{Metriche derivate}: accuratezza complessiva, percentuali di veri positivi, veri negativi, falsi positivi e falsi negativi, percentuale di risposte corrette e di risposte errate, \textit{precision}, \textit{specificity}, \textit{recall}
    \item \textbf{Log dettagliato}: traccia completa di tutte le interazioni con l'LLM
\end{itemize}

Il sistema calcola in tempo reale le metriche di performance durante l'elaborazione, fornendo un feedback oggettivo sull'efficacia del modello.\\
...
Queste limitazioni, sebbene non compromettano l'utilità della versione per scopi di ricerca e testing, evidenziano la necessità di un'integrazione più robusta con sistemi di produzione, obiettivo che viene affrontato nella terza e ultima versione del progetto.
...
La terza versione di \textit{Sensitive Data Detector} rappresenta l'evoluzione finale del progetto, trasformando il sistema da uno strumento di analisi offline a una soluzione completamente integrata per l'elaborazione dei log in ambiente operativo.

Questa versione si interfaccia direttamente con Graylog sia in modalità \textit{push}, analizzando i log in \textit{stream}, che \textit{pull}, analizzando i log in \textit{batch}, creando un sistema \textbf{\textit{end-to-end}} per il monitoraggio continuo dei log.

\subsubsection{Obiettivi e scopo}
\label{subsubsec:ver3_obiettivi}

L'obiettivo principale della terza versione è fornire una soluzione pronta per l'utilizzo in produzione che permetta l'identificazione automatica di dati sensibili in ambienti operativi reali. Questa implementazione affronta le principali limitazioni delle versioni precedenti introducendo funzionalità chiave per contesti enterprise.

Il sistema implementa innanzitutto l'\textbf{elaborazione \textit{stream} in tempo reale} in modalità \textit{push}, permettendo l'analisi immediata dei log non appena vengono generati dai sistemi, e supporta anche una modalità \textit{pull} per l'\textbf{acquisizione \textit{batch} incrementale} dei log tramite API REST di Graylog.\\
L'interfacciamento nativo con Graylog consente l'inserimento immediato negli stack tecnologici esistenti, mentre l'architettura asincrona garantisce la \textbf{scalabilità operativa} necessaria per gestire volumi elevati di log in produzione. Il sistema include inoltre un \textbf{\textit{feedback loop}} attraverso il quale i risultati delle analisi vengono automaticamente rimandati a Graylog, permettendo di visualizzarli tramite interfaccia integrata. Infine, la \textbf{configurazione centralizzata} consente la gestione unificata di input, processing pipeline e output direttamente attraverso l'interfaccia Graylog.

\subsubsection{Architettura e funzionamento}
\label{subsubsec:ver3_architettura}

L'architettura della terza versione introduce un cambio di paradigma rispetto alle versioni precedenti, passando da un modello client-server a un sistema basato su stream processing. Il sistema opera come un micro-servizio che si posiziona tra Graylog e i modelli LLM, fungendo da ponte per l'analisi dei log.\\
L'implementazione completa è disponibile nel sorgente~\refwithpage{lst:code_ver3}.

Il collegamento con Graylog è disponibile in due modalità operative.\\
In modalità \textbf{\textit{push}}, Graylog inoltra i log al sistema in \textit{stream} tramite output GELF su \texttt{TCP 24367} e l'elaborazione avviene in tempo reale.\\
In modalità \textbf{pull}, il sistema interroga periodicamente le API REST di Graylog su uno stream specifico e recupera in \textit{batch} solo i log generati successivamente all'ultima esecuzione effettuata.

\clearpage

Il flusso di elaborazione in modalità \textit{push} real-time si articola nelle seguenti fasi:
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item \textbf{Analisi statica opzionale}: È possibile abilitare opzionalmente l'utilizzo di espressioni regolari direttamente all'interno di questo passaggio per l'identificazione rapida tramite analisi statica. Questa funzionalità è opzionale in quanto ridondante: la pipeline di Graylog viene già configurata automaticamente con regole di analisi statica tramite regex, rendendo superflua una duplicazione del processo
    \end{enumerate}
...
In modalità \textit{pull} il flusso mantiene la stessa logica di analisi e notifica verso Graylog ma sostituisce la ricezione TCP con una chiamata alle API REST di Graylog per ottenere le righe di log filtrate per intervallo temporale. Al termine di ogni ciclo di elaborazione il sistema salva il \textit{timestamp} corrente, in modo che le acquisizioni successive considerino esclusivamente i log prodotti dopo l'ultima analisi completata.
...
Un esempio di messaggio GELF è riportato nel sorgente~\refwithpage{lst:esempio_gelf}.

\subsubsection{Integrazione con Graylog}
\label{subsubsec:ver3_integrazione}

L'integrazione con Graylog in modalità \textit{push} avviene attraverso una configurazione di input, stream e output che crea una pipeline completa di elaborazione:

\begin{itemize}
    \item \textbf{Input \texttt{TCP 5555}}: Riceve i log originali dai sistemi sorgente
    \item \textbf{Stream di routing}: Indirizza i log verso il Sensitive Data Detector tramite output GELF su porta \texttt{TCP 24367}
    \item \textbf{Input \texttt{TCP 5556}}: Riceve le notifiche di dati sensibili dal sistema di analisi
    \item \textbf{Pipeline di processing}: Applica regole aggiuntive per categorizzazione e notifica
\end{itemize}

In modalità \textit{pull}, invece, il sistema utilizza le API REST di Graylog per leggere i log in base al timestamp dell'ultima esecuzione, mantenendo invariata la fase di analisi con LLM e la successiva notifica dei risultati alla porta \texttt{TCP 5556}.

Questa architettura permette di mantenere separati i log originali dalle notifiche di sicurezza, facilitando la creazione di dashboard specializzate e la configurazione di alert specifici per la conformità ai regolamenti sulla privacy direttamente all'interno di Graylog.
...
La configurazione real-time mantiene l'elaborazione di un messaggio alla volta per semplificare il processo di rilevazione della correttezza delle risposte.\\
...
Il sistema implementa una \textbf{gestione concorrente} che consente l'elaborazione parallela di log provenienti da fonti multiple. La \textbf{resilienza} è garantita attraverso l'isolamento degli errori per singola connessione, evitando che problemi localizzati compromettano il funzionamento globale del sistema. L'\textbf{efficienza delle risorse} viene ottimizzata mediante l'utilizzo di I/O non bloccante, che permette un uso ottimale della CPU. Infine, l'architettura supporta la \textbf{scalabilità orizzontale}, consentendo il deployment di multiple istanze per il \textit{load balancing}.

\subsubsection{Output e tracciabilità}
\label{subsubsec:ver3_output}

Il sistema di output della terza versione è progettato per l'integrazione operativa, producendo messaggi strutturati che vengono inviati direttamente a Graylog.

Il sistema genera \textbf{notifiche in tempo reale}: ogni identificazione di un dato sensibile viene immediatamente segnalata a Graylog. La \textbf{preservazione degli identificatori} mantiene il collegamento con i log originali tramite ID univoci, consentendo una tracciabilità completa. L'output utilizza un \textbf{formato standardizzato} compatibile con le pipeline di processing esistenti, facilitando l'integrazione con altri strumenti. Infine, il sistema implementa un \textbf{logging delle operazioni} che consente la tracciatura completa delle attività ai fini di audit e debugging.
...
Relativamente all'\textbf{utilizzo di modelli personalizzati}, sebbene questi migliorino significativamente le performance rispetto ai modelli base, il sistema non utilizza tecniche di \textit{fine-tuning} avanzate come \textit{LoRA}\footnote{\textit{LoRA} (\textit{Low-Rank Adaptation}) è una tecnica di \textit{fine-tuning} efficiente che modifica solo un sottoinsieme dei parametri del modello e permette di creare un nuovo LLM a partire da uno già addestrato.}, che rappresenterebbe la soluzione ottimale per l'adattamento specifico al dominio dei log. Per ulteriori dettagli, si rimanda alla sezione~\refwithpage{subsubsec:addestramento_lora}.
...
\textit{Sensitive Data Detector} è stato sottoposto a un processo di testing e valutazione durante e dopo le fasi dello sviluppo. La seconda versione del sistema opera su file di log preparati ed \textbf{etichettati} (come spiegato nella sezione~\refwithpage{sec:ver2}), permettendo così di definire un \textit{ground truth} per verificare i risultati ottenuti e condurre un processo di testing sistematico.\\
Dato che la terza versione di \textit{Sensitive Data Detector} è analoga alla seconda versione sia nel funzionamento che nei \textit{Modelfile} utilizzati, essa non è stata sottoposta a un processo di valutazione. La differenza principale tra le due versioni è che la terza analizza i \textbf{flussi di log} provenienti da Graylog in ambiente operativo, sia in tempo reale tramite modalità \textit{push} (\textit{stream}) sia tramite acquisizione incrementale in modalità \textit{pull} (\textit{batch}), rendendo impossibile definire un \textit{ground truth} per verificare i risultati ottenuti. Inoltre, l'integrazione con sistemi reali introduce \textbf{variabili ambientali} non controllabili che comprometterebbero la \textbf{riproducibilità} e l'\textbf{affidabilità} di misure quantitative.

\section{Metodologia di test}
\label{sec:metodologia_test}

La metodologia di valutazione si è evoluta significativamente tra le due versioni principali del sistema, passando da un approccio \textbf{qualitativo} basato su osservazione manuale a un sistema \textbf{quantitativo} automatizzato.

La prima versione ha utilizzato un approccio esplorativo per validare la \textbf{fattibilità} dell'utilizzo di Large Language Model nell'identificazione di dati sensibili. La valutazione si è concentrata sull'analisi \textbf{qualitativa} delle risposte, dove ogni output dell'LLM è stato analizzato manualmente per identificare la presenza di \textbf{falsi positivi} e \textbf{falsi negativi}. Parallelamente, sono stati condotti esperimenti manuali di diverse combinazioni di parametri, come il numero di righe per messaggio e i messaggi per chat, per \textbf{ottimizzare} le configurazioni e massimizzare l'efficacia del sistema. La valutazione dei \textbf{vincoli di memoria} e performance ha permesso di gestire le limitazioni architetturali, mentre l'analisi del comportamento del sistema su dataset di dimensioni crescenti (100 e 1000 righe) ha fornito indicazioni sulla \textbf{scalabilità} della soluzione.

L'approccio della prima versione ha evidenziato problematiche critiche legate alla gestione del \textit{context window} limitato dei modelli, portando allo sviluppo di strategie di \textbf{segmentazione} e \textbf{ripristino} delle conversazioni.

La seconda versione ha introdotto un \textit{framework} di valutazione quantitativa basato su \textit{ground truth} annotato manualmente. La metodologia ha previsto la \textbf{preparazione} del dataset attraverso l'annotazione manuale di file di log con \textbf{etichette} binarie (\textit{Y/N}) per ogni riga, indicando la presenza o assenza di dati sensibili.\\
L'elaborazione \textbf{automatizzata} implementa un sistema di confronto automatico tra le risposte del modello e il \textit{ground truth}, consentendo il calcolo delle \textbf{metriche} di veri positivi, veri negativi, falsi positivi e falsi negativi e delle \textbf{metriche derivate} accuratezza, \textit{precision}, \textit{specificity} e \textit{recall}, descritte nella sezione~\refwithpage{sec:metriche_test}. Infine, le valutazioni \textbf{comparative} permettono l'analisi sistematica di modelli differenti con configurazioni standardizzate.

\section{Metriche di valutazione}
\label{sec:metriche_test}

Il sistema di valutazione utilizza metriche standard di \textbf{classificazione binaria}, adattate al contesto specifico dell'identificazione di dati sensibili nei log.

Le metriche \textbf{primarie} utilizzate per la valutazione sono elencate nella tabella~\refwithpage{tab:metriche_primarie}.
...
Dalle metriche primarie sono state calcolate le metriche \textbf{derivate} elencate nella tabella~\refwithpage{tab:metriche_derivate}.
...
\begin{table}[h!]
    ...
    \begin{tabular}{|p{0.25\textwidth}|p{0.25\textwidth}|p{0.4\textwidth}|}
        ...
        \hline
        Perc. Falsi Positivi & $\frac{FP}{TP + TN + FP + FN} \times 100$ & Incidenza di righe non sensibili classificate come sensibili \\
        \hline
        Perc. Falsi Negativi & $\frac{FN}{TP + TN + FP + FN} \times 100$ & Incidenza di righe sensibili non identificate \\
        \hline
        Accuratezza & $\frac{TP + TN}{TP + TN + FP + FN} \times 100$ & Percentuale complessiva di classificazioni corrette \\
        \hline
        \textit{Precision} & $\frac{TP}{TP + FP} \times 100$ & Percentuale di righe classificate come sensibili che sono effettivamente sensibili \\
        \hline
        \textit{Specificity} & $\frac{TN}{TN + FP} \times 100$ & Percentuale di righe effettivamente non sensibili correttamente classificate come tali dal modello \\
        \hline
        \textit{Recall} & $\frac{TP}{TP + FN} \times 100$ & Percentuale di righe sensibili correttamente identificate dal modello \\
        \hline
    \end{tabular}
...
Nel contesto applicativo, i \textbf{falsi negativi} rappresentano un rischio di sicurezza maggiore rispetto ai falsi positivi, in quanto comportano la \textbf{mancata identificazione} di dati sensibili effettivamente presenti nei log. Di conseguenza, la \textit{recall} assume un ruolo particolarmente \textbf{critico}, mentre la \textit{precision} quantifica il costo operativo legato ai falsi allarmi generati dal sistema. La \textit{specificity}, complementare alla \textit{precision}, misura l'accuratezza nell'identificazione delle righe non sensibili, fornendo una valutazione \textbf{completa} delle capacità classificatorie del modello.

\section{Risultati sperimentali}
\label{sec:risultati_sperimentali}

Le valutazioni sono state condotte sulle seguenti \textbf{architetture} hardware:
\begin{itemize}
    \item \textbf{Apple M1 Max}: 32 GB memoria unificata, 32 GPU Core
    \item \textbf{AMD RX6950XT}: 16 GB VRAM, 80 Compute Unit\footnote{Compute Unit: \url{https://rocm.docs.amd.com/projects/rocprofiler-compute/en/latest/conceptual/compute-unit.html}} con 5.120 Vector ALU\footnote{Vector ALU (Arithmetic Logic Unit): \url{https://rocm.docs.amd.com/projects/HIP/en/latest/understand/hardware_implementation.html}}
\end{itemize}


La valutazione della prima versione, come spiegato nella sezione~\refwithpage{sec:metodologia_test}, ha lo scopo di fornire un'analisi \textbf{preliminare} sulla fattibilità dell'approccio e sulla definizione di parametri in grado di gestire le \textbf{limitazioni} hardware. I risultati sono riportati nella tabella~\refwithpage{tab:risultati_v1}.
...
\begin{table}[h!]
    ...
    \end{tabular}
    \caption{Risultati (test v.1, Apple M1 Max)}
    \label{tab:risultati_v1}
\end{table}

La seconda versione ha introdotto metriche quantitative \textbf{precise} permettendo un \textbf{confronto} sistematico, come mostrato nelle tabelle~\refwithpage{tab:risultati_v2_m1} e \refwithpage{tab:risultati_v2_6950} e nelle figure~\ref{fig:grafico_accuratezza_v2_m1}, \refwithpage{fig:grafico_accuratezza_v2_6950}, \ref{fig:grafico_precision_v2_m1}, \refwithpage{fig:grafico_precision_v2_6950}, \ref{fig:grafico_specificity_v2_m1}, \refwithpage{fig:grafico_specificity_v2_6950}, \ref{fig:grafico_recall_v2_m1}, \refwithpage{fig:grafico_recall_v2_6950}, \ref{fig:grafico_tempo_v2} e \refwithpage{fig:grafico_tempo_v2_6950}.

Lo script utilizzato per automatizzare il processo di valutazione della seconda versione è disponibile nel sorgente~\refwithpage{lst:code_testing_ver2}.

\begin{table}[t!]
    \centering
    \csvreader[
        tabular=|l|r|r|r|r|r|r|r|r|r|,
        table head=\hline \textbf{Modello} & \textbf{Righe} & \textbf{Tempo} & \textbf{TN} & \textbf{TP} & \textbf{FN} & \textbf{FP} & \textbf{\textit{Precis.}} & \textbf{\textit{Specif.}} & \textbf{\textit{Recall}} \\ \hline,
        late after line=\\ \hline,
    ]{test_results_m1_v2.csv}{model=\model, righe=\righe, tempo=\tempo, tn=\tn, tp=\tp, fn=\fn, fp=\fp, precision=\precision, specificity=\specificity, recall=\recall}
    {\model & \righe & \tempo & \tn\% & \tp\% & \fn\% & \fp\% & \precision\% & \specificity\% & \recall\%}
    \caption{Risultati (test v.2, Apple M1 Max)}
    \label{tab:risultati_v2_m1}
\end{table}
...
\begin{table}[t!]
    ...
    \csvreader[
        ...
        table head=\hline \textbf{Modello} & \textbf{Righe} & \textbf{Tempo} & \textbf{TN} & \textbf{TP} & \textbf{FN} & \textbf{FP} & \textbf{\textit{Precis.}} & \textbf{\textit{Specif.}} & \textbf{\textit{Recall}} \\ \hline,
        late after line=\\ \hline,
    ]{test_results_6950_v2.csv}{model=\model, righe=\righe, tempo=\tempo, tn=\tn, tp=\tp, fn=\fn, fp=\fp, precision=\precision, specificity=\specificity, recall=\recall}
    {\model & \righe & \tempo & \tn\% & \tp\% & \fn\% & \fp\% & \precision\% & \specificity\% & \recall\%}
    \caption{Risultati (test v.2, AMD RX6950XT)}
    \label{tab:risultati_v2_6950}
\end{table}
...
\begin{figure}[p]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            % Accuratezza su 100 righe
            \addplot table[
                x=model,
                y=accuracy,
                col sep=comma,
                restrict expr to domain={\thisrow{righe}}{99:101}
            ] {test_results_m1_v2.csv};

            % Accuratezza su 200 righe
            \addplot table[
                x=model,
                y=accuracy,
                col sep=comma,
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Confronto dell'accuratezza (test v.2, Apple M1 Max)}
    \label{fig:grafico_accuratezza_v2_m1}
\end{figure}
...
\begin{figure}[p]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Confronto dell'accuratezza (test v.2, AMD RX6950XT)}
    \label{fig:grafico_accuratezza_v2_6950}
\end{figure}
...
\begin{figure}[p]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Confronto della \textit{Precision} (test v.2, Apple M1 Max)}
    \label{fig:grafico_precision_v2_m1}
\end{figure}
...
\begin{figure}[p]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Confronto della \textit{Precision} (test v.2, AMD RX6950XT)}
    \label{fig:grafico_precision_v2_6950}
\end{figure}
...
\begin{figure}[p]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Confronto della \textit{Specificity} (test v.2, Apple M1 Max)}
    \label{fig:grafico_specificity_v2_m1}
\end{figure}
...
\begin{figure}[p]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Confronto della \textit{Specificity} (test v.2, AMD RX6950XT)}
    \label{fig:grafico_specificity_v2_6950}
\end{figure}
...
\begin{figure}[p]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Confronto della \textit{Recall} (test v.2, Apple M1 Max)}
    \label{fig:grafico_recall_v2_m1}
\end{figure}
...
\begin{figure}[p]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Confronto della \textit{Recall} (test v.2, AMD RX6950XT)}
    \label{fig:grafico_recall_v2_6950}
\end{figure}
...
\begin{figure}[p]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Confronto del tempo impiegato (test v.2, Apple M1 Max)}
    \label{fig:grafico_tempo_v2}
\end{figure}
...
\begin{figure}[p]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Confronto del tempo impiegato (test v.2, AMD RX6950XT)}
    \label{fig:grafico_tempo_v2_6950}
\end{figure}
...
I risultati raggiunti durante la fase di sperimentazione e testing mostrati nel \textbf{Capitolo~\refwithpage{chap:test}} confermano la fattibilità dell'utilizzo di LLM per l'identificazione di dati sensibili, in tempo reale, in un ambiente di produzione, evidenziando l'importanza della selezione del modello appropriato in base ai requisiti specifici di \textbf{accuratezza}, \textbf{velocità} e \textbf{risorse disponibili}.

Il presente lavoro ha dimostrato che l'adozione di un'\textbf{architettura ibrida} basata sulla combinazione di espressioni regolari e Large Language Model costituisce un approccio \textbf{efficace} e concretamente \textbf{applicabile} in contesti produttivi. L'integrazione di questi due paradigmi di analisi ha permesso di sfruttare i vantaggi di entrambe le tecnologie: le \textbf{espressioni regolari} garantiscono l'\textbf{identificazione} rapida e deterministica di \textit{pattern} strutturati ben definiti, mentre gli \textbf{LLM} forniscono la \textbf{flessibilità} necessaria per rilevare dati sensibili espressi in linguaggio naturale o secondo formati non convenzionali.

L'implementazione \textbf{progressiva} attraverso tre versioni evolutive ha consentito di validare l'approccio in modo \textbf{sistematico}, partendo da un \textit{proof of concept} fino ad arrivare a una soluzione \textit{production-ready} integrata con piattaforme enterprise di \textit{log management} come Graylog in modalità \textit{push} (\textit{stream}) e \textit{pull} (\textit{batch}). I test condotti hanno evidenziato metriche di \textbf{performance} che confermano la concreta \textbf{usabilità} del sistema in ambienti di \textbf{produzione}, con tempi di risposta compatibili con i requisiti operativi e livelli di accuratezza adeguati per il \textbf{monitoraggio della conformità} alla privacy.

L'esperienza maturata durante lo sviluppo ha inoltre dimostrato come l'\textbf{architettura modulare} proposta permetta di evolvere il sistema in modo \textbf{incrementale}, adattandosi alle specifiche esigenze del \textbf{contesto} applicativo e consentendo \textbf{sostituzioni} o \textbf{aggiornamenti} dei singoli componenti senza compromettere il funzionamento complessivo.

\section{Sviluppi futuri}
\label{sec:sviluppi_futuri}

\subsubsection{Addestramento di \textit{LoRA}}
\label{subsubsec:addestramento_lora}

\textbf{\textit{LoRA}} (\textit{Low-Rank Adaptation}) è una tecnica di \textbf{\textit{fine-tuning}} efficiente che modifica solo un sottoinsieme dei parametri del modello e permette di creare una versione ulteriormente addestrata di un LLM già disponibile e utilizzabile.

A differenza dei \textit{Modelfile} di Ollama, che si limitano a modificare il prompt di sistema e i parametri di configurazione senza alterare i pesi del modello, un \textit{LoRA} esegue un vero e proprio \textbf{addestramento} che aggiorna i parametri del modello stesso. Mentre i \textit{Modelfile} permettono di personalizzare il comportamento del modello attraverso istruzioni testuali, il \textit{LoRA} \textbf{apprende} direttamente dai dati di training, modificando la \textbf{rappresentazione interna} del modello e consentendo un \textbf{adattamento} molto più profondo al dominio specifico.

Tramite questa tecnica è possibile addestrare un modello per un dominio particolare, ad esempio il dominio dei log, senza dover addestrarne uno completamente da zero. Quest'ultima opzione risulta infatti \textbf{impraticabile} per progetti di ricerca accademica a causa dei \textbf{costi} computazionali elevati, dei \textbf{tempi} di sviluppo prolungati, delle \textbf{competenze} multidisciplinari richieste e della necessità di \textbf{\textit{dataset}} di addestramento di dimensioni elevate.

In seguito a un periodo prolungato di utilizzo del sistema in ambiente di produzione e di \textbf{raccolta} sistematica dei dati e dei \textit{report} generati, sarebbe possibile utilizzare questi dati come \textit{dataset} di \textit{training} per addestrare un \textit{LoRA} specificamente \textbf{ottimizzato} per questo caso d'uso.\\
Il \textit{fine-tuning} potrebbe partire da uno dei modelli LLM attualmente utilizzati che ha dimostrato le \textbf{performance} migliori durante la fase di \textit{testing}.

\subsubsection{Integrazione con altri sistemi}
\label{subsubsec:integrazione_sistemi}

La versione finale del progetto è stata sviluppata specificamente per l'\textbf{integrazione} con Graylog e lo script di \textit{setup} dell'ambiente è stato scritto per interagire con questa piattaforma.\\
Un possibile \textbf{sviluppo futuro} consiste nella \textbf{generalizzazione} delle integrazioni attraverso lo sviluppo di un \textbf{protocollo} standard che consenta la creazione di adattatori modulari per diversi sistemi di gestione dei log.

Questa evoluzione permetterebbe di estendere la compatibilità del sistema a piattaforme alternative come \textit{Elasticsearch}\footnote{Elasticsearch \url{https://www.elastic.co/elasticsearch}}, \textit{Splunk}\footnote{Splunk \url{https://www.splunk.com}}, o \textit{Fluentd}\footnote{Fluentd \url{https://www.fluentd.org}}, \textbf{standardizzando} al contempo le interfacce di comunicazione tra il sistema di analisi e i sistemi di \textit{logging}. Inoltre, la definizione di un protocollo comune semplificherebbe la distribuzione del sistema in ambienti enterprise eterogenei e migliorerebbe la \textbf{tracciabilità} e la \textbf{gestione delle notifiche} attraverso un \textit{framework} unificato. Un protocollo di integrazione standardizzato ridurrebbe significativamente la complessità di \textit{deployment} e aumenterebbe l'adozione del sistema in contesti operativi diversificati.
...
\textbf{\textit{SigNoz}}\footnote{SigNoz \url{https://signoz.io}} rappresenta una soluzione moderna \textit{open source} focalizzata sull'osservabilità completa, offrendo un pannello unificato per visualizzare \textit{traces}, metriche e log provenienti da \textit{OpenTelemetry}\footnote{OpenTelemetry \url{https://opentelemetry.io}}. Utilizza \textit{ClickHouse}\footnote{ClickHouse \url{https://clickhouse.com}} come \textit{database} per garantire performance elevate e supporta l'ingestione di dati da oltre 50 sorgenti diverse, rendendolo particolarmente adatto ad ambienti \textit{cloud-native} dove l'integrazione con standard aperti è prioritaria. La piattaforma offre inoltre correlazione automatica tra i diversi segnali telemetrici per un \textit{debugging} più efficace.

\textbf{\textit{Logstash}}\footnote{Logstash \url{https://www.elastic.co/logstash}} si configura come una \textit{pipeline} di \textit{data processing server-side} altamente flessibile con un \textit{framework} estensibile che conta oltre 200 plugin. Il sistema implementa code persistenti per garantire \textit{delivery at-least-once} degli eventi e supporta monitoraggio centralizzato delle \textit{pipeline} attraverso un'interfaccia unificata, risultando ideale in scenari dove è necessaria una pre-elaborazione complessa dei dati.

\textbf{\textit{Fluentd}}\footnote{Fluentd \url{https://www.fluentd.org}} è un \textit{data collector} \textit{open source} caratterizzato da un'architettura basata su \textit{plugin} che conta oltre 500 estensioni disponibili. Progetto della \textit{Cloud Native Computing Foundation} (\textit{CNCF})\footnote{CNCF \url{https://www.cncf.io}}, fornisce un \textit{layer} di \textit{logging} unificato che consente di raccogliere log da sorgenti eterogenee e inoltrarli a destinazioni multiple, disaccoppiando le sorgenti dati dai sistemi \textit{backend}.

\textbf{\textit{Syslog-ng}}\footnote{Syslog-ng \url{https://www.syslog-ng.com}}, implementazione avanzata del protocollo \textit{Syslog}, fornisce funzionalità di raccolta, elaborazione e salvataggio dei log con particolare enfasi sulla compatibilità con sistemi \textit{legacy} e sulla conformità agli standard di \textit{compliance}.

Infine, \textbf{\textit{Apache Flume}}\footnote{Apache Flume \url{https://flume.apache.org}} si posiziona come un servizio distribuito specializzato nella raccolta, aggregazione e spostamento di grandi volumi di \textit{streaming event data} attraverso un modello basato su \textit{data flows}.

L'\textbf{integrazione} di \textit{Sensitive Data Detector} con queste piattaforme alternative richiederebbe l'\textbf{adattamento} del \textit{layer} di comunicazione e la \textbf{definizione} delle \textit{pipeline} di elaborazione, mantenendo invariata la logica di \textbf{analisi} basata su LLM e regex.
...
\lstinputlisting[
caption=Script di testing automatico della versione 2,
label=lst:code_testing_ver2,
language=Bash,
literate={è}{{\`e}}1 {à}{{\`a}}1 {ò}{{\`o}}1 {ù}{{\`u}}1 {ì}{{\`i}}1
]{"../ollama extractor v2/test-all.sh"}
