...
Utilizza la programmazione asincrona con \texttt{asyncio} per gestione concorrente delle connessioni, il protocollo GELF per comunicazione nativa con la piattaforma di gestione log e un server TCP non bloccante per la ricezione dei log in tempo reale.
...
La terza versione di \textit{Sensitive Data Detector} rappresenta l'evoluzione finale del progetto, trasformando il sistema da uno strumento di analisi offline a una soluzione completamente integrata per l'elaborazione in tempo reale dei log.
...
Dato che la terza versione di \textit{Sensitive Data Detector} è analoga alla seconda versione sia nel funzionamento che nei \textit{Modelfile} utilizzati, essa non è stata sottoposta a un processo di valutazione. L'unica differenza tra le due versioni è che la terza analizza i \textbf{flussi di log} provenienti da Graylog in \textbf{tempo reale}, rendendo impossibile definire un \textit{ground truth} per verificare i risultati ottenuti. Inoltre, l'integrazione con sistemi reali introduce \textbf{variabili ambientali} non controllabili che comprometterebbero la \textbf{riproducibilità} e l'\textbf{affidabilità} di misure quantitative.
