...
L'architettura proposta consiste in un sistema di analisi continua dei log, progettato per ricevere flussi di registrazioni in tempo reale, elaborarle individualmente e classificarle in base alla presenza o assenza di dati sensibili. Il sistema opera in modalità \textit{streaming}, processando ciascuna riga di log non appena questa viene acquisita, restituendo un indicatore dell'esito dell'analisi.
...
Il sistema è progettato per riconoscere diverse tipologie di dati sensibili, tra cui informazioni personali identificabili come i PII, credenziali di accesso e token di autenticazione, indirizzi email e informazioni di contatto, e chiavi crittografiche e certificati.
...
In modalità \textbf{pull}, il sistema interroga periodicamente le API REST di Graylog su uno stream specifico e recupera in \textit{batch} solo i log generati dopo l'ultima esecuzione registrata.
...
Il sistema genera \textbf{notifiche real-time} per cui ogni identificazione di dato sensibile produce immediatamente una notifica a Graylog. La \textbf{preservazione degli identificatori} mantiene il collegamento con i log originali tramite ID univoci, consentendo una tracciabilità completa. L'output utilizza un \textbf{formato standardizzato} compatibile con le pipeline di processing esistenti, facilitando l'integrazione con altri strumenti. Infine, il sistema implementa un \textbf{logging delle operazioni} che consente la tracciatura completa delle attività ai fini di audit e debugging.
...
Tramite questa tecnica è possibile addestrare un modello per un dominio specifico, ad esempio il dominio dei log, senza dover addestrarne uno completamente da zero. Quest'ultima opzione risulta infatti \textbf{impraticabile} per progetti di ricerca accademica a causa dei \textbf{costi} computazionali elevati, dei \textbf{tempi} di sviluppo prolungati, delle \textbf{competenze} multidisciplinari richieste e della necessità di \textbf{\textit{dataset}} di addestramento di dimensioni elevate.
