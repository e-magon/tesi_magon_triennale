...
\clearpage
\section{Large Language Models nell'analisi dei log}
...
L'approccio proposto integra un sistema ibrido che combina l'uso di espressioni regolari con l'utilizzo di \textit{Large Language Models}, creando una soluzione ibrida per l'identificazione di dati sensibili in tempo reale.
...
Ollama\footnote{Ollama \url{https://ollama.com}} è un software open-source progettato per semplificare l'esecuzione \textbf{locale} di Large Language Models.\\
...
La prima implementazione costituisce un \textit{proof of concept} sviluppato per dimostrare la fattibilità dell'utilizzo di Large Language Models nell'identificazione automatica di dati sensibili all'interno dei file di log.
...
La prima versione di \textit{Sensitive Data Detector} rappresenta un \textit{proof of concept} sviluppato per dimostrare la fattibilità dell'utilizzo di Large Language Models nell'identificazione automatica di dati sensibili all'interno dei file di log.\\
...
Il sistema è progettato per riconoscere diverse tipologie di dati sensibili, tra i quali informazioni personali identificabili come i PII, credenziali di accesso e token di autenticazione, indirizzi email e informazioni di contatto, e chiavi crittografiche e certificati.
...
Questa strategia offre vantaggi significativi: garantisce \textbf{consistenza} mantenendo il prompt di sistema sempre presente nel contesto del modello ed eliminando la variabilità dovuta alla gestione manuale, migliora l'\textbf{efficienza} riducendo la lunghezza dei messaggi inviati e ottimizzando l'utilizzo del context window, permette la \textbf{specializzazione} ottimizzando ogni modello con prompt specifici per le sue caratteristiche architetturali e supporta il \textbf{reasoning avanzato} per modelli con capacità di ragionamento che utilizzano tag speciali come \texttt{<think>}.
...
\begin{table}[h!]
    ...
    \begin{tabular}{|p{0.25\textwidth}|p{0.25\textwidth}|p{0.4\textwidth}|}
        ...
        \hline
        \textit{Specificity} & $\frac{TN}{TN + FP} \times 100$ & Percentuale di righe classificate come non sensibili che sono effettivamente non sensibili \\
        \hline
...
\textbf{\textit{LoRA}} (\textit{Low-Rank Adaptation}) è una tecnica di \textbf{\textit{fine-tuning}} efficiente che modifica solo un sottoinsieme dei parametri del modello e permette di creare una versione ulteriormente addestrata di un LLM già disponibile ed utilizzabile.
