...
\end{figure}
\TODO{TODO vedere se si riesce a far stare nella pagina prima}
...
L'obiettivo principale di questa seconda implementazione è duplice: da un lato migliorare la scalabilità del sistema per gestire volumi di log più elevati, dall'altro introdurre un \texit{framework} di valutazione automatica per misurare obiettivamente le performance dei modelli LLM.
...
\label{chap:test}
\todo{TODO evidenziare parole chiave + corsivo termini tecnici/inglesi}

\textit{Sensitive Data Detector} è stato sottoposto a un processo di testing e valutazione durante e dopo le fasi dello sviluppo. La seconda versione del sistema opera su file di log preparati ed etichettati (come spiegato nella sezione~\refwithpage{sec:ver2}), permettendo così di definire un \textit{ground truth} per verificare i risultati ottenuti e condurre un processo di testing sistematico.\\
Dato che la terza versione di \textit{Sensitive Data Detector} è analoga alla seconda versione sia nel funzionamento che nei \textit{Modelfile} utilizzati, essa non è stata sottoposta a un processo di testing. L'unica differenza tra le due versioni è che la terza analizza i flussi di log provenienti da Graylog in tempo reale, rendendo impossibile definire un \textit{ground truth} per verificare i risultati ottenuti. Inoltre, l'integrazione con sistemi reali introduce variabili ambientali non controllabili che comprometterebbero la riproducibilità e l'affidabilità di misure quantitative.
...
La prima versione ha utilizzato un approccio esplorativo per validare la fattibilità dell'utilizzo di Large Language Models nell'identificazione di dati sensibili. Il testing si è concentrato sull'\textbf{analisi qualitativa delle risposte}, dove ogni output del modello LLM è stato analizzato manualmente per identificare la presenza di falsi positivi e falsi negativi. Parallelamente, sono stati condotti test manuali di diverse combinazioni di parametri, come il numero di righe per messaggio e i messaggi per chat, per l'\textbf{ottimizzazione delle configurazioni} e massimizzare l'efficacia del sistema. La valutazione dei vincoli di memoria e performance ha permesso di gestire le limitazioni \textbf{hardware}, mentre l'analisi del comportamento del sistema su dataset di dimensioni crescenti (100 e 1000 righe) ha fornito indicazioni sulla \textbf{scalabilità} della soluzione.
...
La seconda versione ha introdotto un framework di testing quantitativo basato su \textit{ground truth} annotato manualmente. La metodologia ha previsto la \textbf{preparazione del dataset} attraverso l'annotazione manuale di file di log con etichette binarie (Y/N) per ogni riga, indicando la presenza o assenza di dati sensibili.\\
L'\textbf{elaborazione automatizzata} implementa un sistema di confronto automatico tra le risposte del modello e il \textit{ground truth}, consentendo il \textbf{calcolo delle metriche} di veri positivi, veri negativi, falsi positivi e falsi negativi e delle metriche derivate accuratezza, \textit{precision}, \textit{specificity} e \textit{recall}, descritte nella sezione~\refwithpage{sec:metriche_test}. Infine, i \textbf{test comparativi} permettono la valutazione sistematica di modelli LLM differenti con configurazioni standardizzate.
...
Dalle metriche primarie sono state calcolate le metriche derivate elencate nella tabella~\refwithpage{tab:metriche_derivate}.
...
Nel contesto applicativo, i falsi negativi rappresentano un rischio di sicurezza maggiore rispetto ai falsi positivi, in quanto comportano la mancata identificazione di dati sensibili effettivamente presenti nei log. Di conseguenza, la \textit{recall} assume un ruolo particolarmente critico, mentre la \textit{precision} quantifica il costo operativo legato ai falsi allarmi generati dal sistema. La \textit{specificity}, complementare alla \textit{precision}, misura l'accuratezza nell'identificazione delle righe non sensibili, fornendo una valutazione completa delle capacità classificatorie del modello.

\section{Risultati sperimentali}
\label{sec:risultati_sperimentali}

I test sono stati condotti sulle seguenti architetture hardware:
\begin{itemize}
...
Il test della prima versione, come spiegato nella sezione~\refwithpage{sec:metodologia_test}, ha lo scopo di fornire una valutazione preliminare sulla fattibilità dell'approccio e sulla definizione di parametri in grado di gestire le limitazioni hardware. I risultati sono riportati nella tabella~\refwithpage{tab:risultati_v1}.
...
La seconda versione ha introdotto metriche quantitative precise permettendo un confronto sistematico, come mostrato nelle tabelle~\refwithpage{tab:risultati_v2_m1} e \refwithpage{tab:risultati_v2_6950} e nelle figure~\refwithpage{fig:grafico_accuratezza_v2_m1}, \refwithpage{fig:grafico_accuratezza_v2_6950}, \refwithpage{fig:grafico_precision_v2_m1}, \refwithpage{fig:grafico_precision_v2_6950}, \refwithpage{fig:grafico_specificity_v2_m1}, \refwithpage{fig:grafico_specificity_v2_6950}, \refwithpage{fig:grafico_recall_v2_m1} e \refwithpage{fig:grafico_recall_v2_6950}.

Lo script utilizzato per automatizzare il processo di testing della seconda versione è disponibile nel sorgente~\refwithpage{lst:code_testing_ver2}.
...
I risultati raggiunti durante la fase di sperimentazione e testing mostrati nel \textbf{Capitolo~\refwithpage{chap:test}} confermano la fattibilità dell'utilizzo di LLM per l'identificazione di dati sensibili, in tempo reale in un'ambiente di produzione, evidenziando l'importanza della selezione del modello appropriato in base ai requisiti specifici di accuratezza, velocità e risorse disponibili.

Il presente lavoro ha dimostrato che l'adozione di un'\textbf{architettura ibrida} basata sulla combinazione di espressioni regolari e Large Language Model costituisce un approccio efficace e concretamente applicabile in contesti produttivi. L'integrazione di questi due paradigmi di analisi ha permesso di sfruttare i vantaggi di entrambe le tecnologie: le \textbf{espressioni regolari} garantiscono l'identificazione rapida e deterministica di pattern strutturati ben definiti, mentre gli \textbf{LLM} forniscono la flessibilità necessaria per rilevare dati sensibili espressi in linguaggio naturale o secondo formati non convenzionali.

L'implementazione progressiva attraverso tre versioni evolutive ha consentito di validare l'approccio in modo sistematico, partendo da un proof of concept fino ad arrivare a una soluzione \textit{production-ready} integrata con piattaforme enterprise di log management come Graylog. I test condotti hanno evidenziato metriche di performance che confermano la \textbf{concreta usabilità} del sistema in ambienti di produzione, con tempi di risposta compatibili con i requisiti operativi e livelli di accuratezza adeguati per il monitoraggio della conformità alla privacy.

L'esperienza maturata durante lo sviluppo ha inoltre dimostrato come l'architettura modulare proposta permetta di evolvere il sistema in modo incrementale, adattandosi alle specifiche esigenze del contesto applicativo e consentendo sostituzioni o aggiornamenti dei singoli componenti senza compromettere il funzionamento complessivo.
...
LoRA (Low-Rank Adaptation) è una tecnica di fine-tuning efficiente che modifica solo un sottoinsieme dei parametri del modello e permette di creare una versione ulteriormente addestrata di un LLM già disponibile ed utilizzabile.

A differenza dei \textit{Modelfile} di Ollama, che si limitano a modificare il prompt di sistema e i parametri di configurazione senza alterare i pesi del modello, un LoRA esegue un vero e proprio addestramento che aggiorna i parametri del modello stesso. Mentre i \textit{Modelfile} permettono di personalizzare il comportamento del modello attraverso istruzioni testuali, il LoRA apprende direttamente dai dati di training, modificando la rappresentazione interna del modello e consentendo un adattamento molto più profondo al dominio specifico.

Tramite questa tecnica è possibile addestrare un modello per un dominio specifico, ad esempio il dominio dei log, senza dover addestrarne uno completamente da zero. Quest'ultima opzione risulta infatti impraticabile per progetti di ricerca accademica a causa dei costi computazionali elevati, dei tempi di sviluppo prolungati, delle competenze multidisciplinari richieste e della necessità di dataset di addestramento di dimensioni elevate.
...
Questa evoluzione permetterebbe di estendere la compatibilità del sistema a piattaforme alternative come Elasticsearch\footnote{Elasticsearch \url{https://www.elastic.co/elasticsearch}}, Splunk\footnote{Splunk \url{https://www.splunk.com}}, o Fluentd\footnote{Fluentd \url{https://www.fluentd.org}}, standardizzando al contempo le interfacce di comunicazione tra il sistema di analisi e i sistemi di logging. Inoltre, la definizione di un protocollo comune semplificherebbe la distribuzione del sistema in ambienti enterprise eterogenei e migliorerebbe la tracciabilità e la gestione delle notifiche attraverso un framework unificato. Un protocollo di integrazione standardizzato ridurrebbe significativamente la complessità di deployment e aumenterebbe l'adozione del sistema in contesti operativi diversificati.
...
L'integrazione di \textit{Sensitive Data Detector} con queste piattaforme alternative richiederebbe l'adattamento del layer di comunicazione e la definizione delle pipeline di elaborazione, mantenendo invariata la logica di analisi basata su LLM e regex.
...
\todo{TODO vedere se fare indice delle figure e delle tabelle}
